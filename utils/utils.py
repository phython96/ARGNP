...
def overall(scores, targets):
    ...
    scores = scores.max(dim=1)[1]
    return accuracy_score(targets.cpu().detach().numpy(), scores.cpu().detach().numpy())
...
def genotype_type(args, arch_para_type, arch_topo_type, type):
    ...
    link           = [ [] for i in range(args.basic.nb_nodes + 1) ]
    for Si, Vj, Ek, L in arch_topo_type:
        link[Si].append((Vj, Ek, arch_para_type[L]))
    for i_node in range(1, args.basic.nb_nodes + 1):
